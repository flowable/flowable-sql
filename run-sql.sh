...
if [[ "$1" == "" ]] ; then
    ...
    cd flowable-sql-6.4.2
    if [[ "$DATABASE" == "oracle" ]] ; then
    	mvn -PgenerateSql -Ddatabasewithschema=$DATABASE -Duser.timezone=GMT -DoldVersion=6.4.1 -DnewVersion=6.4.2 clean install
    	mvn -PgenerateCreateSql -Ddatabasewithschema=$DATABASE -Duser.timezone=GMT clean install
    	mvn -PcombineUpgradeSql -Ddatabasewithschema=$DATABASE -Duser.timezone=GMT -DoldVersion=6.4.1 -DnewVersion=6.4.2 clean install
    	...
    	mvn -PgenerateSql -Ddatabase=$DATABASE -DoldVersion=6.4.1 -DnewVersion=6.4.2 clean install
    	mvn -PgenerateCreateSql -Ddatabase=$DATABASE clean install
    	mvn -PcombineUpgradeSql -Ddatabase=$DATABASE -DoldVersion=6.4.1 -DnewVersion=6.4.2 clean install
